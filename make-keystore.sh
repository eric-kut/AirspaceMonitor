#!/usr/bin/env bash
# One-time release keystore generation for AirspaceMonitor.
...
if [ -f airspacemonitor.keystore ]; then
  echo "airspacemonitor.keystore already exists — not overwriting it."
  exit 1
fi
...
echo "  storeFile=airspacemonitor.keystore"
echo "  storePassword=<the keystore password you entered>"
echo "  keyAlias=airspacemonitor"
