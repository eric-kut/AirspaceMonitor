...
private fun describe(profile: ca.trafficwatcher.domain.Profile): String {
    ...
    val watch = profile.watch?.let { w ->
        when (w.mode) {
            ca.trafficwatcher.domain.WatchMode.OFFSET ->
                " · watch +${w.offsetHkm} km / +%.0f %s".format(w.offsetV, w.offsetVUnit.name)
            ca.trafficwatcher.domain.WatchMode.FOLLOW_PHONE ->
                " · watch ${w.radiusKm} km"
            ca.trafficwatcher.domain.WatchMode.FIXED_CIRCLE ->
                " · watch ${w.radiusKm} km circle"
            ca.trafficwatcher.domain.WatchMode.POLYGON ->
